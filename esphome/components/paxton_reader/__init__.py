...
async def to_code(config):
    ...
    cg.add(var.last_card_ts   .set_parent(last_card))
    cg.add(var.card_type_ts   .set_parent(card_type))
    cg.add(var.card_colour_ts .set_parent(card_colour))
    cg.add(var.bit_count_s    .set_parent(bit_count))
    cg.add(var.reading_bs     .set_parent(reading))

    cg.add(var.net2_bits    .set((config[CONF_NET2_BITS])))
    cg.add(var.switch2_bits .set((config[CONF_SWITCH2_BITS])))
    cg.add(var.debounce_us  .set((config[CONF_DEBOUNCE_US])))
